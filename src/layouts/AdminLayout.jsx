...
export default function AdminLayout() {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-950">
    ...
    <header className="sticky top-0 z-40 border-b border-slate-800 bg-slate-950 px-5 py-4 text-white lg:hidden"><div className="flex items-center justify-between"><div><p className="text-sm font-extrabold">Hakim Plus</p><p className="text-xs text-emerald-300">Pharmacy operations</p></div><span className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs font-bold text-emerald-300">Staff</span></div></header>
    <div className="mx-auto flex min-h-screen max-w-[1700px]"><aside className="hidden w-72 shrink-0 bg-slate-950 p-6 text-white lg:flex lg:flex-col"><div><p className="text-lg font-extrabold">Hakim Plus</p><p className="mt-1 text-xs font-bold uppercase tracking-[0.16em] text-emerald-300">Pharmacy operations</p></div><nav className="mt-10 space-y-2" aria-label="Staff operations">{navigation.map((item) => <NavLink key={item.to} to={item.to} end={item.end} className={navClass}>{item.label}</NavLink>)}</nav><div className="mt-auto rounded-2xl border border-white/10 bg-white/5 p-4"><p className="text-xs text-slate-400">Signed in as</p><p className="mt-1 truncate text-sm font-bold">{auth.user?.email}</p><p className="mt-2 text-xs capitalize text-emerald-300">{roles.join(", ").replaceAll("_", " ")}</p><button className="mt-4 text-sm font-bold text-white hover:text-emerald-300" type="button" onClick={logout}>Log out</button></div></aside><div id="admin-main-content" className="min-w-0 flex-1 pb-20 lg:pb-0" tabIndex="-1"><Outlet /></div></div>
