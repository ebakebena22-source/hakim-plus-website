...
export default function AdminLayout() {
  ...
  const navigation = [
    ...
    ...(canManageOrders ? [{ to: "/admin/orders", label: "Orders & delivery" }, { to: "/admin/completed-orders", label: "Completed orders" }] : []),
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-950">
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 flex gap-1 overflow-x-auto border-t border-slate-800 bg-slate-950 p-2 lg:hidden" aria-label="Mobile staff navigation">{navigation.map((item) => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `grid min-h-14 min-w-28 shrink-0 place-items-center rounded-xl px-2 text-center text-[11px] font-bold ${isActive ? "bg-emerald-500 text-slate-950" : "text-slate-300"}`}>{item.label}</NavLink>)}</nav>
  </div>;
}
