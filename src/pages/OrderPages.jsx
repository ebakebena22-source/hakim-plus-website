...
export function OrderDetailPage() {
  ...
  return <main className={pagePadding}><Link className="text-sm font-bold text-slate-500 hover:text-emerald-700" to="/dashboard/orders">← Orders</Link>{state.error && <div className="mt-5"><ErrorMessage message={state.error} /></div>}<div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold text-emerald-700">Medication order</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">{order.orderNumber || id}</h1><p className="mt-2 text-sm text-slate-600">For {order.beneficiary?.fullName || order.beneficiaryName}</p></div><span className={`self-start rounded-full px-4 py-2 text-sm font-bold ${failed ? "bg-red-50 text-red-800" : "bg-emerald-50 text-emerald-800"}`}>{order.statusLabel || String(order.status).replaceAll("_", " ")}</span></div><section className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-6 sm:p-8"><h2 className="text-lg font-bold">Order progress</h2>{failed && <p className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-800">{order.customerStatusNote || "Hakim Plus could not complete this delivery. Support will provide the next step."}</p>}<CustomerOrderTracker status="delivery" failed={failed} completed={completed} /></section><div className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]"><section className="rounded-[2rem] border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold">Order items</h2><div className="mt-5 divide-y divide-slate-100">{order.items?.map((item,index) => <div key={item.id || index} className="flex justify-between gap-4 py-4 text-sm"><div><p className="font-bold">{item.medicationName}</p><p className="mt-1 text-slate-500">{item.strength} · {item.quantity} {item.unitLabel}</p></div><p className="font-bold">{formatMinorAmount(item.lineTotalMinor, order.currency)}</p></div>)}</div><div className="mt-5 flex justify-between border-t border-slate-200 pt-5 text-lg font-bold"><span>Total paid</span><span>{formatMinorAmount(order.amountMinor, order.currency)}</span></div></section><aside className="space-y-6"><section className="rounded-[2rem] border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold">Delivery information</h2><dl className="mt-5 space-y-4 text-sm"><div><dt className="text-slate-500">Beneficiary</dt><dd className="mt-1 font-bold">{order.beneficiary?.fullName || order.beneficiaryName}</dd></div><div><dt className="text-slate-500">Address</dt><dd className="mt-1 whitespace-pre-wrap font-bold">{order.delivery?.address || order.deliveryAddress}</dd></div><div><dt className="text-slate-500">Instructions</dt><dd className="mt-1 whitespace-pre-wrap font-semibold">{order.delivery?.instructions || "None"}</dd></div></dl>{order.delivery?.proofViewUrl && <a className="mt-5 inline-flex rounded-xl border border-emerald-600 px-4 py-2 text-sm font-bold text-emerald-700" href={order.delivery.proofViewUrl} target="_blank" rel="noreferrer">View delivery confirmation</a>}</section><section className="rounded-[2rem] border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold">Important updates</h2>{order.timeline?.length ? <ol className="mt-5 space-y-4">{order.timeline.map((event,index) => <li key={event.id || index} className="border-l-2 border-emerald-200 pl-4"><p className="text-xs text-slate-500">{formatDate(event.createdAt || event.timestamp)}</p><p className="mt-1 text-sm font-bold">{event.customerLabel || event.label}</p>{event.note && <p className="mt-1 text-sm text-slate-600">{event.note}</p>}</li>)}</ol> : <p className="mt-3 text-sm text-slate-600">Payment confirmation is the first order event.</p>}</section></aside></div>{completed && <section className="mt-6 rounded-[2rem] border border-emerald-200 bg-emerald-50 p-6"><h2 className="text-lg font-bold text-emerald-950">Need the same medications again?</h2><p className="mt-2 text-sm leading-6 text-emerald-900">This creates a new request for pharmacy review. It never repeats the charge or automatically reuses an old prescription.</p><button className="mt-5 rounded-xl bg-emerald-700 px-4 py-3 text-sm font-bold text-white" type="button" disabled={reordering} onClick={requestAgain}>{reordering ? "Creating new request…" : "Request again"}</button></section>}
  </main>;
}
