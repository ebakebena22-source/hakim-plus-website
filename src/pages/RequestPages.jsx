...
const stageLabels = ["Request submitted", "Pharmacy review", "Quote", "Payment", "Preparation", "Delivery", "Completed"];
...
const stageByStatus = {
  submitted: 0,
  under_pharmacy_review: 1,
  additional_information_required: 1,
  contacting_beneficiary: 1,
  prescription_verification: 1,
...
export function RequestsPage() {
  ...
  useEffect(() => {
    let active = true;
    requestsApi.list().then((result) => { if (active) setState({ status: "ready", requests: result.requests || result.items || [], error: "" }); }).catch((error) => { if (active) setState({ status: "error", requests: [], error: error.message }); });
    return () => { active = false; };
  }, []);
...
export function RequestDetailPage() {
  ...
  if (state.status === "error") return <main className={pagePadding}><ErrorMessage message={state.error} /></main>;
  const request = state.request;
  const currentStage = stageByStatus[request.status] ?? 0;
  ...
  return <main className={pagePadding}><Link className="text-sm font-bold text-slate-500 hover:text-emerald-700" to="/dashboard/requests">← All requests</Link><div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold text-emerald-700">Medication request</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">{request.requestNumber || id}</h1><p className="mt-2 text-sm text-slate-600">{request.beneficiary?.fullName || request.beneficiaryName} · Submitted {formatDate(request.submittedAt || request.createdAt)}</p></div><span className={`inline-flex self-start rounded-full px-4 py-2 text-sm font-bold ${terminal ? "bg-red-50 text-red-800" : "bg-emerald-50 text-emerald-800"}`}>{labelStatus(request.status, request.statusLabel)}</span></div>
    {["quote_ready", "awaiting_customer_approval"].includes(request.status) && <section className="mt-8 rounded-[2rem] border border-amber-300 bg-amber-50 p-6 sm:flex sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-amber-800">Action required</p><h2 className="mt-2 text-xl font-bold text-amber-950">Your pharmacy quote is ready</h2><p className="mt-2 text-sm text-amber-900">Review availability, pricing, fees, and expiration before deciding.</p></div><Link className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-amber-800 px-5 text-sm font-bold text-white sm:mt-0" to={`/dashboard/requests/${encodeURIComponent(id)}/quote`}>Review quote</Link></section>}
    {request.status === "awaiting_payment" && <section className="mt-8 rounded-[2rem] border border-emerald-300 bg-emerald-50 p-6 sm:flex sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-emerald-800">Action required</p><h2 className="mt-2 text-xl font-bold text-emerald-950">Secure payment is required</h2><p className="mt-2 text-sm text-emerald-900">Your approved quote is ready for provider-hosted checkout.</p></div><Link className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-emerald-700 px-5 text-sm font-bold text-white sm:mt-0" to={`/dashboard/requests/${encodeURIComponent(id)}/payment`}>Continue to payment</Link></section>}
    {(request.orderPath || request.order?.publicId || request.order?.id) && <section className="mt-8 rounded-[2rem] border border-blue-200 bg-blue-50 p-6 sm:flex sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-blue-800">Paid order</p><h2 className="mt-2 text-xl font-bold text-blue-950">Follow preparation and delivery</h2><p className="mt-2 text-sm text-blue-900">This request now has a separate fulfillment order.</p></div><Link className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-blue-800 px-5 text-sm font-bold text-white sm:mt-0" to={request.orderPath || `/dashboard/orders/${encodeURIComponent(request.order.publicId || request.order.id)}`}>Track order</Link></section>}
    <section className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-6 sm:p-8"><h2 className="text-lg font-bold">Request progress</h2>{terminal ? <p className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-800">{request.statusNote || "Hakim Plus could not continue this request. Contact support if you need help."}</p> : <ol className="mt-6 grid gap-3 sm:grid-cols-7">{stageLabels.map((label,index) => <li key={label} className={`rounded-xl p-3 text-xs font-bold ${index < currentStage ? "bg-emerald-600 text-white" : index === currentStage ? "border-2 border-emerald-600 bg-emerald-50 text-emerald-900" : "bg-slate-100 text-slate-400"}`}><span className="block text-[10px] opacity-70">{index < currentStage ? "Complete" : index === currentStage ? "Current" : `Step ${index + 1}`}</span><span className="mt-1 block">{label}</span></li>)}</ol>}</section>
