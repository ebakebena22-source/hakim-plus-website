...
export function BeneficiaryFormPage() {
  ...
  return <main className={pagePadding}><PageHeader eyebrow="Beneficiary profile" title={editing ? "Edit beneficiary" : "Add a beneficiary"} description="Only provide information Hakim Plus needs to verify requests, contact the beneficiary, and arrange delivery." />
    <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
      ...
      <Section title="Basic information"><Field id="fullName" label="Full name" required value={form.fullName} error={errors.fullName} onChange={(event) => update("fullName", event.target.value)} /><Field id="relationship" label="Relationship to you (optional)" placeholder="Mother, father, sibling…" value={form.relationship} onChange={(event) => update("relationship", event.target.value)} /><Field id="age" label="Approximate age" inputMode="numeric" required value={form.age || ""} error={errors.age} onChange={(event) => update("age", event.target.value)} /><SelectField id="sex" label="Sex" value={form.sex} onChange={(event) => update("sex", event.target.value)}><option value="female">Female</option><option value="male">Male</option><option value="prefer-not-to-provide">Prefer not to provide</option></SelectField><Field id="email" label="Email (optional)" type="email" value={form.email || ""} error={errors.email} onBlur={(event) => setErrors((current) => ({ ...current, email: emailError(event.target.value, { required: false }) }))} onChange={(event) => update("email", event.target.value)} /></Section>
      <Section title="Contact information"><Field id="phone" label="Phone number" type="tel" inputMode="tel" placeholder="+251 911 234 567" required value={form.phone} error={errors.phone} onBlur={(event) => setErrors((current) => ({ ...current, phone: phoneError(event.target.value) }))} onChange={(event) => update("phone", event.target.value)} /><Field id="alternativePhone" label="Alternative phone (optional)" type="tel" inputMode="tel" placeholder="+251 911 234 567" value={form.alternativePhone || ""} error={errors.alternativePhone} onBlur={(event) => setErrors((current) => ({ ...current, alternativePhone: phoneError(event.target.value, { required: false }) }))} onChange={(event) => update("alternativePhone", event.target.value)} /></Section>
      <Section title="Delivery location"><Field id="country" label="Country" required value={form.country} error={errors.country} onChange={(event) => update("country", event.target.value)} /><Field id="city" label="City" required value={form.city} error={errors.city} onChange={(event) => update("city", event.target.value)} /><div className="sm:col-span-2"><TextAreaField id="deliveryAddress" label="Delivery address" required value={form.deliveryAddress} error={errors.deliveryAddress} onChange={(event) => update("deliveryAddress", event.target.value)} /></div></Section>
...
export function BeneficiaryDetailPage() {
  ...
  return <main className={pagePadding}><PageHeader eyebrow="Beneficiary profile" title={beneficiary.fullName} description={[beneficiary.relationship, [beneficiary.city, beneficiary.country].filter(Boolean).join(", ")].filter(Boolean).join(" · ")} action={<div className="flex gap-3"><Link className="inline-flex min-h-11 items-center rounded-xl border border-slate-300 px-4 text-sm font-bold text-slate-700" to={`/dashboard/beneficiaries/${encodeURIComponent(id)}/edit`}>Edit</Link><Link className="inline-flex min-h-11 items-center rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white" to={`/dashboard/requests/new?beneficiary=${encodeURIComponent(id)}`}>New request</Link></div>} />
