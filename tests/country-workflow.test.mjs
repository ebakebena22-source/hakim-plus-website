...
test("customer tracking has four stages while fulfillment exposes only delivery outcomes", async () => {
  ...
  assert.match(api, /request\.latestUpdate = customerRequestUpdate\(internalStatus, orderStatus, statusHistory, cancelledByCustomer\)/);
  ...
  assert.match(customerRequests, /!request\.completed && <section/);
  assert.match(customerRequests, /request\.cancelledByCustomer \? "Order cancelled by customer"/);
  assert.match(api, /cancelledBy: "customer"/);
  assert.match(api, /cancelledBy: "staff"/);
  assert.match(api, /ae\.event_type='request\.cancelled_by_customer'/);
  assert.match(api, /o\.status order_status/);
  assert.match(api, /Payment confirmed and order created\./);
  assert.match(api, /Your payment receipt is awaiting verification\./);
  assert.match(api, /Your order is out for delivery\./);
  assert.match(api, /Delivery was not completed\. Please review the latest delivery update\./);
