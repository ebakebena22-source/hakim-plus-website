...
test("diaspora care landing page stores leads before firing Meta conversion", async () => {
  ...
  assert.match(pixel, /typeof window\.fbq !== "function"/);
  assert.match(pixel, /fbq\("track", "Lead"\)/);
  assert.doesNotMatch(pixel, /diasporaPhone|patientPhone|careNeed/);
  assert.doesNotMatch(api, /sendMetaLead|event_name: "Lead"/);
  assert.ok(page.indexOf("await createDiasporaCareRequest") < page.indexOf("trackDiasporaCareLead()"));
